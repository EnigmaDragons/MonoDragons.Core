...
namespace MonoDragons.Core.AudioSystem
{
    public class SmartSample : ISampleProvider, IDisposable
    {
        ...
        public SmartSample(Sound sound, string fileName) : this(sound, new AudioFileReader(fileName)) { }

        public SmartSample(Sound sound, AudioFileReader reader)
        {
            _sound = sound;
            _reader = reader;
        ...
        public int Read(float[] buffer, int offset, int sampleCount)
        {
            if (!_sound.IsPlaying)
                return 0;
            _volumeProvider.Volume = _sound.Mute ? 0 : _sound.Volume * MasterVolume.GetVolume(_sound.Type);
